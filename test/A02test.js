...
QUnit.test('Testing compute Function with several inputs', function (assert) {
    assert.equal(temperatureConverterOfF(54), 12.222222222222221, 'Tested with two positive integers');
    assert.equal(temperatureConverterOfF(-1),-18.333333333333332,'Tested with two negative integers');
    //assert.equal(App.temperatureConverterOfF("hello","hi"), 0,'Tested with strings');  
    assert.equal(temperatureConverterOfF("08"),0,'Tested with one zero and one negative number');
    //assert.throws(function () { App.compute(null); }, /The given argument is not a number/, 'Passing in null correctly raises an Error');
  
   
});
QUnit.test('Testing compute Function with several inputs', function (assert) {
    assert.equal(temperatureConverterOfC(200), 392, 'Tested with two positive integers');
    assert.equal(temperatureConverterOfC(-40), -40,'Tested with two negative integers');
    //assert.equal(App.temperatureConverterOfC("hello","hi"), 0,'Tested with strings');  
    assert.equal(temperatureConverterOfC("string"),0,'Tested with one zero and one negative number');
    //assert.throws(function () { App.compute(null); }, /The given argument is not a number/, 'Passing in null correctly raises an Error');
